...
class WechatPayService extends Service {
  ...
  async handleNotify(headers, body) {
    ...
    // 1. 手动签名校验 (使用微信支付公钥)
    const is_valid = service.common.wechat.verifySignature(headers, body);
    if (!is_valid) {
    ...
    const decrypted_data = service.common.wechat.decrypt(
      resource.ciphertext,
  ...
  async createOrderSP(params) {
    ...
    try {
      ...
      const pay_sign = service.common.wechat.calculateJSSign(params.sp_appid || this.config.wechatPay.sp_appid, time_stamp, js_nonce_str, package_str);
   ...
   * [服务商模式] 添加分账接收方
   */
  async addProfitSharingReceiver(sub_mchid, sp_name) {
    const { service } = this;
    const url = 'https://api.mch.weixin.qq.com/v3/profitsharing/receivers/add';
    
    const encrypted_name = service.common.wechat.encryptSensitive(sp_name);
  ...
  async queryOrderSP(out_trade_no, sub_mchid) {
    const { service } = this;
    const result = await service.common.wechat.queryOrderSP(out_trade_no, sub_mchid);
    return result;
  }
  ...
  async closeOrderSP(out_trade_no, sub_mchid) {
    return await this.service.common.wechat.closeOrderSP(out_trade_no, sub_mchid);
  }
}
