...
class StaffController extends Controller {
  ...
  async index() {
    ...
    const result = await service.admin.staff.list(params);
  ...
  async setPermissions() {
    const { ctx, service } = this;
    const { staff_id, permission_codes } = ctx.request.body;

    if (!staff_id || !Array.isArray(permission_codes)) {
      return ctx.helper.fail(ctx, 400, '参数错误，权限码需为阵列');
    }

    const result = await service.admin.staff.setPermissions(staff_id, permission_codes);
