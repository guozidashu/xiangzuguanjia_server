    ...
    <view class="payment-record-container">
        ...
        <view class="bill-summary-card">
            <view class="amount-section">
                ...
                <text class="amount-value">¥{{ $tools.formatMoney(unpaidAmount) }}</text>
            </view>
            ...
            <view class="info-grid">
                ...
                <view class="info-item">
                    ...
                    <text class="info-value success">¥{{ $tools.formatMoney(billData.paid_amount || 0) }}</text>
                </view>
            </view>
        </view>
        ...
        <view class="form-card">
            <view class="form-section">
                ...
                <view class="form-item">
                    <text class="label required">支付金额</text>
                    <input class="input" v-model="formData.paid_amount" type="digit" placeholder="请输入支付金额" />
                </view>
                <view class="form-tip">最多可支付 ¥{{ $tools.formatMoney(unpaidAmount) }}</view>

                <!-- 收款方式 -->
                <view class="form-item">
                    <text class="label required">收款方式</text>
                ...
                <!-- 收款日期 -->
                <view class="form-item">
                    <text class="label required">收款日期</text>
                    <picker mode="date" :value="formData.paid_date" :end="todayDate" @change="handleDateChange">
                        <view class="picker-value">
                            {{ formData.paid_date }}
                        </view>
                    </picker>
                ...
                <!-- 收款时间 -->
                <view class="form-item">
                    <text class="label required">收款时间</text>
                    <picker mode="time" :value="formData.paid_time" @change="handleTimeChange">
                        <view class="picker-value">
                            {{ formData.paid_time }}
                        </view>
                    </picker>
        ...
        <view class="footer-actions">
            ...
            <view class="action-btn submit" @click="handleSubmit">确认支付</view>
        </view>
    </view>
</template>
...
export default {
    ...
    methods: {
        ...
        initData() {
            ...
            this.formData.paid_date = this.todayDate;
            this.formData.paid_time = this.currentTime;
        },
        ...
        async loadBillData() {
            try {
                ...
                // 设置默认支付金额为待支付金额
                this.formData.paid_amount = this.unpaidAmount.toFixed(2);
            } catch (error) {
         ...
         */
        handleDateChange(e) {
            this.formData.paid_date = e.detail.value;
        },
         ...
         */
        handleTimeChange(e) {
            this.formData.paid_time = e.detail.value;
        },
        ...
        validateForm() {
            // 验证支付金额
            const amount = parseFloat(this.formData.paid_amount);
            if (!this.formData.paid_amount || isNaN(amount)) {
                uni.showToast({ title: '请输入支付金额', icon: 'none' });
                return false;
            }
            if (amount <= 0) {
                uni.showToast({ title: '支付金额必须大于0', icon: 'none' });
                return false;
            }
            if (amount > this.unpaidAmount) {
                uni.showToast({ title: '支付金额不能超过待支付金额', icon: 'none' });
                return false;
            }
            ...
            // 验证支付日期
            if (!this.formData.paid_date) {
                uni.showToast({ title: '请选择支付日期', icon: 'none' });
                return false;
            }

            // 验证支付时间
            if (!this.formData.paid_time) {
        ...
        async handleSubmit() {
            ...
            try {
                ...
                const params = {
                    id: this.billId,
                    paid_amount: parseFloat(this.formData.paid_amount),
                    payment_method: this.formData.payment_method,
                    paid_date: `${this.formData.paid_date} ${this.formData.paid_time}:00`, // 组合日期和时间
                    transaction_id: this.formData.transaction_id || undefined,
                    notes: this.formData.notes || undefined
                };

                await uni.api.payBill(params);
